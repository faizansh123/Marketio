...
export default function DemoPage() {
    ...
    const handleGenerateVideo = async () => {
        ...
        try {
            ...
            // Check if it's a mock video (quota exceeded)
            if (data.mock) {
                // We can use a toast or just a state to show a badge. 
                // For now, let's just log it and maybe we can add a visual indicator below.
                console.warn(data.note);
                setIsMock(true); // Set isMock state
            }
            ...
            fetch("/api/export-to-sheets", {
                ...
                body: JSON.stringify({
                    ...
                    isMock: data.mock // Pass this along so we can maybe tag it in sheets too if we wanted
                })
            }).then(r => r.json())
                                ...
                                {!videoUrl ? (
                                    ...
                                    <div className="space-y-4">
                                        <div className="aspect-[9/16] w-full max-w-[320px] mx-auto rounded-xl overflow-hidden border border-white/20 bg-black relative shadow-2xl">
                                            {isMock && (
                                                <div className="absolute top-2 left-2 right-2 z-10 bg-yellow-500/90 text-black text-xs font-bold px-3 py-2 rounded-lg text-center shadow-lg backdrop-blur-sm">
                                                    ⚠️ Mock Video (API Quota Exceeded)
                                                </div>
                                            )}
